...
string ClearCommand::execute(const string& command,
                             ...
                             const vector<string>& opts,
                               Signals* my_signals) {
    string output = "\nYou may input \"clear --help\" for more information.";
    // command name
    // description
    ...
    if (opts.size() > 1) {
        output = "\"clear\" can get only one unnecessary option." + output;
        return output;
    }
    ...
    for (string opt: opts) {
        ...
        if (got_opts[opt] == "") {
            output = "Flag " + opt + " wasn`t found." + output;
            return output;
        }
    }
    // CHECKING ARGUMENTS
    ...
    if (args.size() > 0) {
        output = "\"clear\" doesn`t have any required arguments." + output;
        return output;
    }
    // EXECUTING COMMAND
