...
unordered_map<char, pair<int, int>> gen_vertices_coords(const Graph& G) {
    ...
    for (auto v1_neighbours: graph) {
        ...
        while (true) {
            // строим всевозможные прямые из добавленных в граф вершин
            bool isExit = false;    // на выход из внешнего цикла for

            // проверенные пары, чтобы не строить одни и те же прямые по несколько раз
            unordered_map<char, unordered_map<char, bool>> checked_vertices;
